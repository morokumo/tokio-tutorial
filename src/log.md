...
* AsyncRead と AsyncWrite
    ...
    - AsyncWrite
        - std::io::Write　を継承
        ノンブロッキングI/Oオブジェクト
        バイトの書き込みができなかった場合，スレッドをブロックするのではなく．エラーを返す．

        - poll_write関数は以下のいずれかを返す
            - Poll::Ready(Ok(n))
                - nバイトのデータの書き込みが即時完了したことを示している．
            - Poll::Pending
                - 与えられたバッファからデータの書き込みが行われなかったことを示している．
                これは，現在書き込みが可能では
                現在のFutureタスクはオブジェクトが書き込み可能になった時に保留が解除されるようにスケジュールされている．
                I/Oオブジェクトが書き込み可能になった時に通知を受け取ることができる．

            - Poll::Ready(Err(e))
                - 標準的なI/Oエラー
        - このトレイトの重要な点として，writeメソッドはfutureタスクのコンテキスト内でのみ動作する．

## Framing

* RESP（REdis Serialization Protocol）
    - 以下のデータタイプをサポートするシリアライズプロトコル
        - Simple Strings シンプルな文字列
        - Errors エラー
        - Integer 整数
        - Bulk Strings バルク文字列
        - Arrays 配列
    ...
    - Bulk Strings とは
        - 最大512MBの長さのバイナリセーフな文字列で表される
        - Nullの値が存在する．　表現は次の通り，　`$-1\r\n`
        これは，Null　Bulk　Stringと呼ばれる．
...
* BufWriter
    - AsyncWriteを実装したものを直接操作するのは非常に非効率？
    - これはデータのバッファをインメモリで保持し，大規模で頻度の低いバッチでWriterにデータを書きこむ．
    - 同じファイルやネットワークソケットに対して少量の書き込みと読み込みを繰り返すプログラムの速度向上につながる．
    - 逆に，大容量や，少ない回数しか行わない場合には速度向上に繋がらない．
    - また，Vec<u8>などのメモリ上にあるものに対する場合にも速度向上に繋がらない．

    - **writeを実行したとしても，中のAsyncWriterのwriteが実行されるのではなく，バッファへの書き込みが行われる．**
    - **バッファが規定のサイズになった場合にまとめてAsyncWriterのwriteが実行される．**
    - **規定のサイズに満たない時に，バッファの内容を書き出す場合は，Writerがドロップする前に手動でflush()を呼び出す必要がある．**

* バイナリセーフとは？
    - NULLバイトなどどんなバイト列だとしても正しく処理されること

## Async in depth

* std::future:: Future
    - 非同期の計算を表す．
    - futureは計算が終わっていない可能性のある値
    - この値（非同期の値）は利用可能になるのを待つ間に他の処理を続けることを可能にする．

    - pollメソッド
        - futureのコアメソッド
        - 最終的な値に解決しようとする
        - このメソッドは値の準備ができていなくてもブロックしない
        - ブロックはしないが，再度ポーリングすることで，前進（値の取得？）が可能になった時に実行されるようにスケジュールされる．
        - pollに渡されるコンテキストは，現在のタスクを実行するためのHandleであるWakerを提供する．

    - なお，通常futureを使用する場合は，値をawaitする．

    - **futureはバックグラウンドで実行されている計算のことではない**
    futureは「計算自体」を指す

* polling （ポーリング）とは？
    - 一定の条件を満たした場合に処理を行う処理方式？
    - 複数のプログラムに対して順番に定期的に問い合わせを行うことで実現
    - 競合を回避したり、送受信の準備状況を判断したり、処理を同期したりするために用いられる

* Pinについて
    - 良さげ　https://www.youtube.com/watch?v=ii6Sek4q2oM
    - メモリ上の位置にデータを固定する型
    - 動かないことが保障されていると便利な場合がある
    - つまりメモリ上の位置が変わらない
    - 実用例としては，自己参照構造体
    - 自身へのポインタを持つオブジェクトを移動するとポインタが無効になり未定義動作を引き起こす可能性を生む
    - Pin<P>は任意のポインタ型Pがメモリ内の固定された位置に存在することを示す．
    - したがって，メモリ上の位置を変更することはできず，ドロップされるまでそのメモリは解放されない．

    - Rustにおいては，デフォルトですべの型は移動可能
    - 全ての型で値を渡すことが可能

* Associated Types （関連型？）
    - https://doc.rust-jp.rs/rust-by-example-ja/generics/assoc_items/the_problem.html

    - trait と 型のプレースホルダーを結びつけるもの
    - トレイトのメソッド定義がシグニチャでプレースホルダーの型を使用できるようにする
    - ジェネリックなトレイトを実装した場合，そのトレイトを使用するものは全ての方を明記**しなければならない**
...
* Box<T>
    - Rustにおいては全ての値はデフォルトでスタック領域に割り当てられる．
    Boxを用いることで，任意の値をヒープ領域に置くことが可能．
    正確には，ヒープに置かれた値へのスマートポインタ
...
* https://tech-blog.optim.co.jp/entry/2019/07/05/173000
 [2021/06/25]
https://ja.wikipedia.org/wiki/ポーリング_(情報)
